...
#define __GPIO_OUTPUT_SET(__nr, __to) do { if ((__nr) >= 32) { gpio_exp_setgpio((__nr)-32, (__to) ? true : false); } else { GPIO_OUTPUT_SET(__nr, __to); } } while (0)
...
static const uint8_t ASCII[][5] =
{
  ...
  ,{0x89, 0x92, 0xff, 0x92, 0x89} // 80     [bjdooks]
  ,{0x00, 0x00, 0x00, 0x00, 0x00} // 81     [bjdooks]
};
...
PCD8544_lcdCharacter(char character) {
  ...
  if (character<0x20 || character>0x81) {
    character = '?';
  }
  int index = 0;
  for (; index < 5; index++) {
...
void ICACHE_FLASH_ATTR
PCD8544_lcdPrint(const char *characters) {
  while (*characters) {
...
// bjd - all invocations seem to have msbFirst as 'true'
// bjd - datasheet claims DIN sampled on rising edge (think mode0 should do)
...
PCD8544_shiftOut8(bool msbFirst, uint8_t data) {
  ...
  if (msbFirst) {
    for (bit = 7; bit>=0; bit--) {
      ...
      __GPIO_OUTPUT_SET(pinSclk, 0);
      __GPIO_OUTPUT_SET(pinSdin, dataBit);
      os_delay_us(CLOCK_HIGH_TIME);
      __GPIO_OUTPUT_SET(pinSclk, 1);
      os_delay_us(CLOCK_HIGH_TIME);
      __GPIO_OUTPUT_SET(pinSclk, 0);
    }
  } else {
...
PCD8544_lcdWrite8(bool dc, uint8_t data) {
  __GPIO_OUTPUT_SET(pinDc, dc);
  if (pinSce>=0) {
    __GPIO_OUTPUT_SET(pinSce, LOW);
  }
  PCD8544_shiftOut8(true, data);
  if (pinSce>=0) {
    __GPIO_OUTPUT_SET(pinSce, HIGH);
  }
  os_delay_us(CLOCK_HIGH_TIME);
}
...
PCD8544_init(PCD8544_Settings *settings) {
  ...
  if (pinReset>=0)
    __GPIO_OUTPUT_SET(pinReset, HIGH);
  if (pinSce>=0)
    __GPIO_OUTPUT_SET(pinSce, HIGH);
  __GPIO_OUTPUT_SET(pinDc, HIGH);
  __GPIO_OUTPUT_SET(pinSdin, LOW);
  __GPIO_OUTPUT_SET(pinSclk, LOW);
...
PCD8544_initLCD(PCD8544_Settings *settings) {
  ...
  if (pinReset>=0){
    os_delay_us(10000);
    __GPIO_OUTPUT_SET(pinReset, LOW);
